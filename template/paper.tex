...
\end{itemize}
由于策略网络$\pi_\theta$需要实体级和层次级强化学习模块共同构建，因此这部分内容将主要在后文的双重代理强化学习交互模块进行介绍，其他部分则在本节进行介绍。
...
\begin{equation}
  \bm{A}_{i}^{add} = \left\{(\hat{r}, \hat{e}) \mid (e_i, \hat{r}, \hat{e}) \notin G, \hat{r} \in R, \hat{e} \in E_i \right\}
\end{equation}
...
\begin{equation}
  \bm{A}_{i}^{add} = \left\{(\hat{r}, \hat{e}) \mid f_\hat{r}(e_i, \hat{e}) \geq T, (e_i, \hat{r}, \hat{e}) \notin G, \hat{r} \in R, \hat{e} \in E_i \right\}
\end{equation}
...
本节在介绍双重代理强化学习交互模块的过程中，首先给出了模块概览并提供了模块结构图。随后介绍了模块提出的协作策略网络和轨迹相似度奖励，主要分析了模块如何通过上述两种内容实现了实体级和层次级强化学习模块的信息交互。最后介绍了双重代理智能体的学习过程，给出了模块的知识推理算法。
...
因此，为了提高知识推理的效果，有必要将上述两种知识推理模型融合起来，实现一种基于双重代理强化学习的知识推理模型。为了实现上述目标，本文设计了双重代理强化学习交互模块。模块的结构如图XXX所示。可以发现，模块的输入是实体级和层次级强化学习模块部分获取的状态信息和动作空间，随后在双重代理强化学习交互模块中通过\textbf{协作策略网络}综合两个模块的信息选择后续动作并进行状态转移。当推理达到最大跳数并获得最终的答案实体后，通过双重代理强化学习交互模块的轨迹相似度奖励评估两个模块智能体是否能够按照相似的轨迹进行推理，并与两个模块自身的奖励函数相结合，为智能体提供奖励。
通过本节提出的双重代理强化学习交互模块，能够实现两种粒度的知识推理模块间的交互，进而增强知识推理效果。
% \begin{figure}
%   \centering
%   \includegraphics[width=0.9\textwidth]{3_DualReinforcementLearning}
%   \caption{双重代理强化学习交互模块结构图}
...
\subsection{轨迹相似度奖励}
在双重代理强化学习交互模块中为了实现实体级和层次级知识推理模块的信息交互，本节还针对奖励函数做了新的设计，即轨迹相似度奖励。
...
\subsection{双重代理知识推理}
本节提出的双重代理强化学习交互模块是知识推理模型LAURA的核心模块，通过协作动作空间、协作策略网络和轨迹相似度奖励，将实体级和层次级知识推理模块结合了起来，确保两种模块能够接受对方信息并沿着对方相似的推理路径进行知识推理，实现了宏观和微观信息的紧密交流和结合，从而能够进行双重代理知识推理。
基于双重代理强化学习的知识推理模型LAURA的训练过程如算法XXX所示。
% 算法X
