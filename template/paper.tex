...
\begin{abstract}{表示学习，知识推理，强化学习，知识图谱}
  ...
  \begin{itemize}
    ...
    模型首先利用层次信息提取模块，采用极坐标表示法编码知识图谱并通过半径坐标捕获知识图谱的层次信息，提高模型对知识信息的表示效果。
    随后采用因果推断模块，对图神经网络模块构建因果图，并采用邻域信息因果干预的方法，综合考虑层次差异得分、决策差异得分、预测置信度得分和推理距离得分等多种因素，评估当前节点是否接收来自邻域节点的信息，提高模型的稳定性。
    同时通过图神经网络模块，利用层次信息提取模块的层次信息作为辅助，采用因果推断模块评估是否接收邻域信息。完成训练后的模型能够将输入的知识三元组转化为知识表示向量，并可以用于后续知识推理工作。
    \item [2.]\textbf{提出一种基于双重智能体强化学习的知识推理模型LAURA：}
    模型采用强化学习框架，能够根据知识图谱中的拓扑结构进行多跳推理，并能够通过显式推理生成具有可解释性的推理路径。
    通过结合知识图谱表示学习模型NORMAN，LAURA模型能够对动作空间进行扩展，从而缓解了知识图谱稀疏性问题。通过利用NORMAN模型的评分函数作为软奖励提供给没有到达目标节点的推理过程，可以缓解知识推理的奖励稀疏问题。
    在知识推理的过程中，实体级和层次级强化学习模块能够分别根据知识图谱中的实体和层次信息进行知识推理，实现微观和宏观两种粒度的知识推理。
    同时，通过双重智能体强化学习交互模块实现上述两种粒度的强化学习模块的交互。通过采用协作动作空间、协作策略网络和轨迹相似度奖励，能够实现模块间的交互，提高知识推理效果，从而实现高质量知识推理。
  \end{itemize}
\end{abstract}
...
自此，完成了对知识图谱表示学习模型NORMAN的图神经网络模块的全部介绍。整个图神经网络模块的算法过程如算法XXX所示。可以发现，图神经网络模块融合了层次信息提取模块和因果推断模块的内容，是NORMAN模型的核心。在图神经网络模块中融合了层次信息抽取模块和因果推断模块的内容，实现了在知识图谱表示过程中维护知识图谱内的层次信息、因果信息和邻域信息，为知识图谱问答等下游任务提供支持。
...
单跳知识推理的典型方法是直接通过知识图谱表示学习模型，采用知识图谱问答的方式实现知识推理，即给定查询三元组$\left(h, r, ?\right)$或$\left(?, r, t\right)$，通过将知识图谱内所有关系或实体作为候选答案，输入到知识图谱表示学习模型中，从而将实体与关系映射到低维向量空间中，并通过得分函数进行打分，随后根据得分结果进行排序，选择得分最高的实体或关系作为答案，并完成知识推理。然而，上述单跳推理方法无法提供推理路径，这使得推理过程缺乏可解释性~\cite{wang2019deeppath}。
...
在图神经网络模块，需要调整的超参数则主要是邻域子图最大距离$L$，需要根据实验需要和不同的知识图谱进行调整。
...
\begin{table}[]
  ...
  \bottomrule[1pt]
  \end{tabular*}
  \caption{知识图谱问答超参数设置情况}
  \label{Hyperparameters1}
\end{table}

\subsection{实验结果与分析}
% 整体结果分析
为了评估模型的表现，本文在FB15K-237、WN18RR和NELL-995数据集上选择了多种知识图谱表示学习模型进行实验。除了本文提出的NORMAN模型，还选择了XXX及XXX等模型作为对比模型，共同开展实验。

在FB15K-237知识图谱上进行的实验结果如表~\ref{Experiment1_FB15K-237}所示。可以发现，XXX。
\begin{table}[]
  \centering
  \begin{tabular*}{0.95\textwidth}{@{\extracolsep{\fill}}lcccc}
  \toprule[1pt]
...
在WN18RR知识图谱上开展的对比实验结果如表~\ref{Experiment1_WN18RR}所示。根据结果可以发现，XXX。
\begin{table}[]
  \centering
  \begin{tabular*}{0.95\textwidth}{@{\extracolsep{\fill}}lcccc}
  \toprule[1pt]
...
在NELL-995知识图谱上进行的实体链接实验对应的结果如表~\ref{Experiment1_NELL-995}所示。根据结果可以发现，XXX。
\begin{table}[]
  \centering
  \begin{tabular*}{0.95\textwidth}{@{\extracolsep{\fill}}lcccc}
  \toprule[1pt]
...
% 任务类别实验
为了评估NORMAN模型对不同关系类别的知识图谱问答的实施效果，本文采用NELL-995数据集并选择其中10种关系作为分析对象，对各个模型的表现结果分析如表~\ref{Experiment1_tasks}所示，表中的得分为MRR。可以发现XXX。
\begin{table}[]
  \centering
  \begin{tabular*}{0.95\textwidth}{@{\extracolsep{\fill}}lccccccc}
  \toprule[1pt]
  任务名称 & A & B & C & D & E & F & NORMAN \\ \hline
  A    &   &   &   &   &   &   &        \\
  B    &   &   &   &   &   &   &        \\
  C    &   &   &   &   &   &   &        \\
  D    &   &   &   &   &   &   &        \\
  ...
  \bottomrule[1pt]
  \end{tabular*}
  \caption{NELL-995数据集关系类别实验结果}
  \label{Experiment1_tasks}
\end{table}

% 消融实验
为了评估模型的各个模块效果，本文还对NORMAN模型的各个模块开展了消融实验。如表~\ref{Experiment1_ablation}所示，展示了消融实验的结果。
\begin{table}[]
  \centering
  \begin{tabular*}{0.95\textwidth}{@{\extracolsep{\fill}}lcccc}
  \toprule[1pt]
...
\begin{table}[]
  ...
  \toprule[1pt]
  \multirow{2}{*}{超参数名称} & \multirow{2}{*}{取值范围} & \multicolumn{3}{c}{最佳超参数}\\ 
                         &                       & FB15K-237-10 & FB15K-237-20 & FB15K-237-50 \\ \hline
  A                      &                       &              &              &              \\
  B                      &                       &              &              &              \\
  C                      &                       &              &              &              \\
  D                      &                       &              &              &              \\
  \bottomrule[1pt]
  \end{tabular*}
  \caption{多跳知识推理FB15K-237衍生数据集超参数设置情况}
  \label{Hyperparameters2_Multihop1}
\end{table}
...
\subsection{实验结果与分析}
在单跳知识推理实验中，实验采用的数据集包括FB15K-237、WN18RR和NELL-995，并选择XXX及XXX等模型作为比较对象。
% 整体结果分析

各个模型在单跳知识推理任务上的实验结果如表XXX所示。可以发现XXX。
\begin{table}[]
  \centering
  \begin{tabular*}{0.95\textwidth}{@{\extracolsep{\fill}}lcccc}
  \toprule[1pt]
...
各个模型在多跳知识推理任务上的表现结果如表XXX所示。可以发现XXX。
\begin{table}[]
  \centering
  \begin{tabular*}{0.95\textwidth}{@{\extracolsep{\fill}}lcccc}
  \toprule[1pt]
...
% 消融实验
随后进行消融实验，评估模型各个模块的重要性。消融实验在FB15K-237-50数据集上开展，设置了五种消融模型，分别是。结果如表~\ref{Experiment2_ablation}所示。可以发现，XXX。
\begin{table}[]
  \centering
  \begin{tabular*}{0.95\textwidth}{@{\extracolsep{\fill}}lcccc}
  \toprule[1pt]
...
% 案例分析
最后对LAURA模型在多跳知识推理任务上的任务执行情况进行案例分析。如表~\ref{Experiment2_CaseStudy}所示，。
\begin{table}[]
  \centering
