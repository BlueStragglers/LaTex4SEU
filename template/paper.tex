...
\begin{abstract}{表示学习，知识推理，强化学习，知识图谱}
  ...
  针对上述问题，本文首先提出一种基于图神经网络的知识图谱表示学习模型，该模型能够利用极坐标编码器捕获知识图谱中的层次结构，并采用图神经网络模型捕获知识图谱中节点的邻域信息进行知识表示和推理，同时通过因果干预调整图神经网络的消息传递过程以提高模型效果。
  ...
  \begin{itemize}
    ...
    同时，通过双重智能体强化学习交互模块提供的协作动作空间、协作策略网络和协作奖励，能够让上述两种粒度的强化学习模块进行信息交互和轨迹协调，进而实现高质量知识推理。
...
% 层次信息提取过程如图\ref{2_NORMAN}所示。可以发现提取过程主要是采用基于极坐标系的编码模型将知识图谱映射到欧氏空间，并通过半径坐标区分不同层次的实体和关系，通过角坐标区分不同类型的实体和关系。
% 图中包含了四种不同类别的节点，但节点$e_2$和$e_4$的半径坐标模量相等，说明两个节点位于同一个层次。
% 通过层次信息提取能够将知识转换成表示向量，同时能够通过半径坐标向量聚类的方式获取实体和关系的层次级别，从而可以为图神经网络模型提供支持。
% \begin{figure}
%   \centering
%   \includegraphics[width=0.7\textwidth]{2_Layers}
%   \caption{层次信息提取过程}
%   \label{2_Layers}
% \end{figure}
...
% 在如图\ref{2_NORMAN}所示的图神经网络模型样例中可以发现，使用模型进行了查询三元组为$(e_s, ?, ?)$知识图谱补全的任务。
...
% 由于邻域信息对知识表示具有重要作用，因此本文构建的图神经网络模块首先会通过递归的方式进行知识图谱子图构建。
...
% 针对上述问题，本节通过因果推断方式~\cite{yao2021survey}分析邻域信息并决定是否接收来自邻域节点的消息，从而避免无关信息的干扰，提高表示效果。
% 同时这里不采用深度学习模型，而是采用SVM（Support Vector Machine，支持向量机）模型构建二元分类器，从而保证模块能够利用少量数据进行充分的模型训练。
...
% \end{figure}
% 如图\ref{2_CausalIntervention}所示，在因果干预步骤中，目标是通过选择是否接收来自特定邻域节点的信息，从而避免无关信息的干扰并提高知识图谱表示学习的效果。
% 为了确定哪些邻域节点消息在消息传递过程中需要屏蔽，本文设计了一种二元分类器，能够根据邻域子图和节点的属性确定是否屏蔽来自邻域节点的信息。
% 该分类器的输入主要包括以下内容：层次差异得分、决策差异得分、预测置信度得分和推理距离得分。
...
% 为了降低二元分类器的复杂性，保证模型能够在知识图谱数据量较少的情况下进行学习和训练，本文选择使用SVM模型训练二元分类器，用于评估是否接收某个邻域节点的信息。
% 设置一个阈值$T$表示SVM进行二元分类的阈值，高于该阈值则接受该邻域节点的消息，否则不接受消息。模型决策过程可以表示如下：
% \begin{equation}
%   \bm{Result} = \left\{\begin{array}{l}
%   \bm{1},\quad p > T, \\
%   \bm{0},\quad p \leq T,
...
% 为了训练SVM二元分类器，需要进行因果干预数据集$D$的构建。构建数据集的过程可以表示为：
% \begin{equation}
%   D = \{((v_h, v_d, v_c, v_l), f) \mid (h, r, t) \in F \cup (h, r, t) \notin F\},\quad f = \operatorname{model}(\bm{\hat{h}}_o)
% \end{equation}
% 其中$\operatorname{model}$表示应用知识图谱表示学习模型进行知识图谱补全实验得到的结果，当正确结果排名在前$K$名时则返回1，否则返回-1。这里将$K$设置为10，从而对应知识图谱补全任务的Hits@10指标。
...
% 综上所述，通过在消息传递过程中结合因果推断方法，能够避免接受无关信息并提高知识表示效果。
% 模型在消息传递时采用一个GRU层作为门控机制，减少GNN模型的限制并维护信息的长期传播。
% 消息传递函数可以表示为：
% \begin{equation}
%   \bm{h_o} = \operatorname{GRU}(\mathbf{W} \cdot （\bm{h_s} + \sum_{\bm{n_i} \in \bm{n}}{\operatorname{SVM}(v_h, v_d, v_c, v_l) \cdot \bm{\alpha} \cdot \bm{n_i}})
%   \label{equation_finalMessagePassing}
% \end{equation}
% 其中的

% 在消息聚合部分融入因果推断模块的二元分类器，用于分析节点邻域是否存在局部性差异，并决定是否接收邻域节点信息。通过随机屏蔽掉一定的邻域消息，并计算对应的得分$v_h, v_d, v_c, v_l$和因果干预向量$\bm{\hat{h_o}}$，随后使用二元分类器计算因果干预后得分$p$，如果高于阈值$T$则选择因果干预后的结果$\bm{\hat{h_o}}$，否则仍然选择保留邻域信息的结果$\bm{h_o}$。这一过程可以表示为：
% \begin{equation}
...
% 同时无法将层次信息提供给因果推断模块，此时因果推断模块放弃层次信息，仅采用其他信息进行因果推断。
...
% ，三个消融模型中，NORMAN(w/o CI)的表现最接近NORMAN模型，这说明因果推断模块对模型的整体效果影响相对较小。但可以发现添加了因果推断模块后，模型依然分别在MRR、Hits@1、Hits@3和Hist@10指标上实现了$2.3\%$、$2.3\%$、$3.1\%$和$1.6\%$的提升，说明在图神经网络的消息传递过程中加入因果推断是具有良好效果的，能够稳定地提升知识图谱表示效果。同时，因果推断的提升相对较小是因为另外两个模块更加重要，在表\ref{Experiment1_ablation}中可以发现
...
%     \includegraphics[width=0.3\textwidth]{2_NORMAN_layer_FB15K237.pdf}
%     \label{Experiment1_layer_a}
%   }
%   \subfloat[WN18RR数据集] {
%     \includegraphics[width=0.3\textwidth]{2_NORMAN_layer_WN18RR.pdf}
%     \label{Experiment1_layer_b}
%   }
%   \subfloat[NELL-995数据集] {
%     \includegraphics[width=0.3\textwidth]{2_NORMAN_layer_NELL995.pdf}
%     \label{Experiment1_layer_c}
%   }
...
% \begin{figure}[t]
%   \centering
%   \includegraphics[width=0.5\textwidth]{2_NORMAN_causalInference.pdf}
...
% \begin{figure}[t]
%   \centering
%   \includegraphics[width=0.7\textwidth]{3_LevelReinforcementLearning}
...
% 构建过程如图\ref{3_LevelKG}所示。基于该知识图谱即可进行层次级智能体的知识推理。
% \begin{figure}
%   \centering
%   \includegraphics[width=0.7\textwidth]{3_LevelKG}
%   \caption{层次级知识图谱构建过程}
%   \label{3_LevelKG}
% \end{figure}
